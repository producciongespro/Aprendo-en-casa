...
$mysqli=conectarDB();
	...
	function registraUsuario($usuario, $pass_hash, $nombre, $pais, $provincia, $sexo, $rangoEdad, $activo, $token, $tipo_usuario){
		$mysqli = conectarDB();
      mysqli_query($mysqli,"INSERT INTO usuarios (nickname, password, nombre, pais, provincia, sexo, edad, correo, activacion, token, id_tipo) VALUES('$usuario', '$pass_hash', '$nombre', '$pais','$provincia', '$sexo', '$rangoEdad', '$usuario@correo.cr', '$activo', '$token', '$tipo_usuario')") or die ("Problemas al a���adir elementos a la BD".mysqli_error($mysqli));
			if ($mysqli) {
				return 1;
			}
		 else {
